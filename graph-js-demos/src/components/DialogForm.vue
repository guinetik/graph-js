            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div
                  ...
                  role="radiogroup"
                  :aria-labelledby="`field-${index}-label`"
                >
                  <label
                    v-for="option in field.options"
                    :key="option.value"
                    ...
                    :class="{
                      'border-green-500 bg-green-50 dark:bg-green-900/20': fieldValues[index] === option.value,
                      'border-gray-300 dark:border-gray-600 hover:border-green-300': fieldValues[index] !== option.value
                    }"
                  >
                    <input
                      type="radio"
                ...
                <!-- Emoji Input -->
                <input
                  v-else-if="field.type === 'emoji'"
                  :id="`field-${index}`"
                  v-model="fieldValues[index]"
                  type="text"
                  :placeholder="field.placeholder || 'Type an emoji 😊'"
                  :required="field.required"
                  maxlength="10"
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-2xl text-center"
                  @keyup.enter="handleConfirm"
                  @keyup.esc="handleCancel"
                />
              </div>
...
const props = defineProps({
   ...
   * @type {Array<{label: string, type?: 'text'|'select'|'radio'|'emoji', required?: boolean, options?: Array<{value: string, text: string, emoji?: string}>, placeholder?: string}>}
   */
  fields: {
    type: Array,
    required: true,
